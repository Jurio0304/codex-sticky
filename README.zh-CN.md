...
面向终端优先工作流的 OpenAI Codex CLI 轻量增强版。
...
Codex Sticky 是 OpenAI Codex CLI 的非官方轻量增强版。项目不隶属于 OpenAI，也不由 OpenAI 维护、赞助或背书。它不是官方 Codex CLI 的完整替代品，而是在官方 Codex CLI 基础上提供更适合终端工作流的交互增强。
...
媒体素材预留在 `docs/assets/readme/` 下。目前 README 只放注释占位符，不引用不存在的图片，避免 GitHub 出现破图。
...
如果你希望先看清楚脚本内容，建议使用第一种方式。安装脚本会下载当前 Linux x86_64 GNU 包，校验 `SHA256SUMS`，并且只写入 `~/.local/bin/codex-sticky`。它不会安装或覆盖名为 `codex` 的二进制。
...
以 `0.137.0-sticky.1` 为例：

- `0.137.0` 是 OpenAI Codex 基线版本。
- `sticky.1` 是该基线上的第 1 个 Sticky 补丁集。

新的 Sticky Release tag 不使用 `v` 前缀。
