...
#[test]
fn sticky_selection_handles_reverse_drag_and_empty_selection() {
    let state = StickyTranscriptState::new(/*enabled*/ true);
    seed_rows(&state, &["alpha", "beta"]);
    ...
    assert!(state.selection.borrow().is_none());
}

#[test]
fn sticky_selection_ignores_out_of_bounds_and_survives_scroll_keys() {
    let state = StickyTranscriptState::new(/*enabled*/ true);
    seed_rows(&state, &["alpha", "beta", "gamma"]);
    ...
    assert!(state.handle_key_event(KeyEvent::from(KeyCode::PageUp)));
    assert!(state.selection.borrow().is_some());
    assert!(state.handle_key_event(KeyEvent::from(KeyCode::PageDown)));
    assert!(state.selection.borrow().is_some());
}

#[test]
fn sticky_selection_clears_on_resize_and_disable() {
    let mut state = StickyTranscriptState::new(/*enabled*/ true);
    seed_rows(&state, &["alpha", "beta"]);
    ...
    seed_rows(&state, &["alpha", "beta"]);
    let _ = state.handle_mouse_event(mouse(
        MouseEventKind::Down(MouseButton::Left),
        /*column*/ 1,
        /*row*/ 0,
    ));
